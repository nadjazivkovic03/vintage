export const translations = {
    en: {
        "POCETNA": "HOME",
      "USLUGE": "SERVICES",
      "VOZILA": "FLEET",
      "O NAMA": "ABOUT US",
      "REZERVACIJA": "BOOK",
      "KONTAKT": "CONTACT",
      "LUKSUZNI PREVOZ": "LUXURY TRANSPORT",
      "Premium vozačke usluge sa stilom":
        "Premium chauffeur services with style",
      "REZERVISI": "BOOK",
      "Dobrodošli u svet vrhunskog luksuza i besprekornog komfora. Naša rent a car ponuda obuhvata pažljivo odabrana luksuzna vozila namenjena onima koji ne pristaju na kompromis. Bilo da putujete poslovno ili privatno, obezbeđujemo iskustvo vožnje koje odiše stilom, sigurnošću i prestižom.":"Step into a world of refined luxury and uncompromising comfort. Our rent-a-car collection features an exclusive selection of premium vehicles for those who demand more. Whether for business or leisure, we deliver an elevated driving experience defined by elegance, safety, and prestige.",
      "PONUDA VOZILA" : "FLEET SELECTION",
      "Vrhunska Vozila" : "Premium Vehicles",
      "Pažljivo odabrana flota najvišeg standarda" : "Carefully selected fleet of premium vehicles",
      "Tačnost" : "Punctuality",
      "Uvek na vreme, bez izuzetaka" : "Always on time, without exception",
        "Sigurnost" : "Safety",
        "Diskretna i bezbedna vožnja u svakom trenutku" : "Discretion and security in every journey",
        "Prevoz prilagođen vašim potrebama i navikama" : "Transportation tailored to your needs and preferences",
        "Personalizovana Usluga" : "Personalized Service",

        "NASE USLUGE": "OUR SERVICES",
    "Obezbedjujemo Vam luksuzan prevoz za svaku priliku":
      "Luxury transportation for every occasion",

    "Aerodrom Transfer": "Airport Transfer",
    "Lagodan prevoz do i od Aerodroma":
      "Comfortable transportation to and from the airport",
    "Pracenje leta": "Flight monitoring",
    "Docekujemo Vas na aerodromu ili na vasoj adresi":
      "Pick-up at the airport or at your address",
    "Preuzimamo prtljag": "Luggage handling",
    "Prevozimo Vas na destinaciju u luksuznom vozilu":
      "Transport to your destination in a luxury vehicle",
    "Dodatna lokacija: +10€": "Additional location: +€10",
    "Cena po vozilu": "Price per vehicle",
    "Cena : €50": "Price: €50",
    "Zakazi": "Book",

    "Rentiranje vozila sa vozačem":
      "Chauffeur Service",
    "Vaš privatni vozač na raspolaganju":
      "Your private driver at your disposal",
    "Odabir datuma": "Date selection",
    "Privatnost": "Privacy",
    "Izbor vozila": "Vehicle selection",
    "Pogledaj cenovnik": "View pricing",
    "Cenovnik": "Pricing",
    "Nazad": "Back",
      "Hvala Vam!": "Thank You!",
      "Kontaktiracemo Vas u najkracem mogucem roku.": "We will contact you as soon as possible.",

    "Iznajmljivanje luksuznog vozila":
      "Luxury Car Rental",
    "Iznajmite luksuzno vozilo za sopstvenu vožnju.":
      "Rent a luxury vehicle for self-drive",
    "Potpuno osiguranje": "Full insurance coverage",
    "Premium vozila": "Premium vehicles",
    "24/7 podrška": "24/7 support",
    "Cenovnik – Bez vozača": "Pricing – Without Driver",

    "NASA VOZILA" : "OUR FLEET",
    "Sa vozačem (8 sati)" : "With Driver (8 hours)",
    "Fiksna cena" : "Fixed Price",
    "Bez vozača" : "Without Driver",
    "dana" : "days",
    "WinCar je kompanija posvećena pružanju vrhunskih usluga luksuznog prevoza i iznajmljivanja premium vozila, namenjena klijentima koji očekuju visok standard, pouzdanost i potpunu diskreciju.":
      "WinCar is a company dedicated to delivering premium luxury transportation and high-end vehicle rental services for clients who expect exceptional standards, reliability, and complete discretion.",

    "Posebnu pažnju posvećujemo svakom detalju — od besprekornog stanja vozila, preko tačnosti i sigurnosti, do profesionalnog odnosa prema svakom klijentu.":
      "We pay special attention to every detail — from the immaculate condition of our vehicles to punctuality, safety, and a professional approach to every client.",

    "Bilo da putujete poslovno ili privatno, WinCar je vaš pouzdan partner za elegantan i bezbrižan prevoz.":
      "Whether you are traveling for business or leisure, WinCar is your trusted partner for elegant and worry-free transportation.",
      
      "REZERVISITE SVOJU VOZNJU" : "BOOK YOUR RIDE",
      "Izaberite uslugu" : "Service selection",
      "Izaberite vozilo" : "Vehicle selection",
      "Vozilo sa vozačem" : "Chauffeur Service",
      "Poseban zahtev" : "Special request",
      "Ime" : "First Name",
      "Prezime" : "Last Name",
      "Telefon" : "Phone",
      "Poruka" : "Message",
      "POSALJI ZAHTEV" : "SEND REQUEST",
      "Kontaktirajte nas za rezervacije i dodatne informacije" : "Contact us for reservations and additional information",
        "Adresa" : "Adress",
        "Beograd, Srbija" : "Belgrade, Serbia",
        "Profesionalni vozač" : "Professional driver",
        "Domet 250km" : "Range 250km",

        "Cena u zavisnosti od broja lokacija" : "Price depending on the number of locations",
        "po dogovoru" : "negotiable",
        "Sa vozačem (8 sati) - domet 250km" : "With Driver (8 hours) - range 250km",
        "POZOVI ODMAH": "CALL NOW",
        "24/7 PODRSKA": "24/7 SUPPORT",
        "AERODROM": "AIRPORT",
        "FULL KASKO": "FULL INSURANCE",
        "Bez skrivenih troškova": "No Hidden Costs",
        "PREMIUM VOZILA": "PREMIUM VEHICLES",
        "Od": "From",
        "dan": "day",
        "Datum": "Date",
        "Vozilo": "Vehicle",
        "Od datuma": "From date",
        "Do datuma": "To date",
        "Whatsapp broj": "Whatsapp number",
        "REZERVACIJA VOZILA": "RESERVATION",
        "Ime i prezime": "First and Last Name",
        "Telefon / WhatsApp": "Phone / WhatsApp",
        "Datum preuzimanja": "Pick-up Date",
        "Datum vraćanja": "Return Date",
        "POŠALJI UPIT": "SEND REQUEST",
        "Automatik": "Automatic",
        "sedišta": "seats"
    }
  };
  